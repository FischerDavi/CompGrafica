...
"   if(keyEnabled)\n"
"   {\n"
"       result += calculateLight(keyLightPos, vec3(1.0), 1.0);\n"
"   }\n"
"   if(fillEnabled)\n"
"   {\n"
"       result += calculateLight(fillLightPos, vec3(0.7), 0.4);\n"
"   }\n"
"   if(backEnabled)\n"
"   {\n"
"       result += calculateLight(backLightPos, vec3(1.0), 0.7);\n"
"   }\n"
...
//atualiza trajetoria
void updateTrajectory(OBJ &obj, float speed)
{
    if (obj.trajectory.empty())
        return;

    glm::vec3 target =
        obj.trajectory[obj.currentPoint];
    ...
    if (distance < 0.1f)
    {
        ...
        if (obj.currentPoint >= obj.trajectory.size())
        {
            obj.currentPoint = 0;
        }
    }
...
int main()
{
    ...
    obj1.trajectory.push_back(glm::vec3(-4.0f, 0.0f, -4.0f));
    obj1.trajectory.push_back(glm::vec3( 4.0f, 0.0f, -4.0f));
    obj1.trajectory.push_back(glm::vec3( 4.0f, 0.0f,  4.0f));
    obj1.trajectory.push_back(glm::vec3(-4.0f, 0.0f,  4.0f));

    obj1.position = obj1.trajectory[0];
    ...
    obj2.trajectory.push_back(glm::vec3(0.0f, 0.0f, -6.0f));
    obj2.trajectory.push_back(glm::vec3(6.0f, 0.0f,  0.0f));
    obj2.trajectory.push_back(glm::vec3(0.0f, 0.0f,  6.0f));
    obj2.trajectory.push_back(glm::vec3(-6.0f,0.0f,  0.0f));

    obj2.position = obj2.trajectory[0];
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        {
            updateTrajectory(objetos[i], 0.03f);
        }
